...
class BarController extends HomeController 
{
    public function index()
    {
        ...
        // 关注人数
        $follow = M('follow')->where('bid='.I('get.id'))->select();
        // 帖子
        $list = M('post')->field('u.name uname, p.title, p.descr, p.state, p.ctime, p.id')->table('qm_user u,qm_post p')->where('p.bid='.I('get.id').' and p.state=1 and p.uid=u.id')->order('p.ctime desc')->page($_GET['p'],5)->select();
        // 分页
        ...
        // 贴吧名人
        $supfans = M('follow')->field('u.name, u.exp, u.picname')->table('qm_user u, qm_follow f')->where('f.bid='.I('get.id').' and f.uid=u.id')->order('u.exp desc')->select();
