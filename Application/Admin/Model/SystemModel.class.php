...
class TypeModel extends Model{
    ...
    */
    public function getAdminCate($id=0, $field=true){
        //获取所有分类信息
        $list = $this->field($field,count(*) count)->select();

        //处理分类信息
        $list =  $this->getHtmlCate($list, $count=$list['count'], $id, 0);
       // V($list);
        //返回
        return $list;
    }
     ...
     * @return [type]         [二维数组]
     */
    private function getHtmlCate($cate,$count='0',$pid=0, $level=0){
        //定义空数组
        $arr = array();
        //遍历数组
        foreach($cate as $v){
            if($v['pid'] == $pid){
                ...
                $v['level'] = $level;
                $arr[] = $v;
                $arr = array_merge($arr,$this->getHtmlCate($cate,$count,$v['id'],$level+1));
            }
        }
